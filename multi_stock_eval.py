...
print("=== Multi-stock NSE universe ===")
print(NSE_TICKERS)

df_raw = YahooDownloader(
    start_date=TRAIN_START,
    end_date=TEST_END,
    ticker_list=NSE_TICKERS,
).fetch_data()
print("Raw rows:", len(df_raw), "tickers:", sorted(df_raw["tic"].unique().tolist()))
...
print("Processed rows:", len(processed), "tickers:", sorted(processed["tic"].unique().tolist()))
...
print(f"Stock dim: {stock_dimension}, State space: {state_space}, Cost: {COST}")
...
model.set_logger(configure(RESULTS_DIR + "/ppo_nse10", ["stdout", "csv"]))
trained = agent.train_model(model=model, tb_log_name="ppo", total_timesteps=50000)
trained.save(TRAINED_MODEL_DIR + "/agent_ppo_nse10")
...
print("\n=== MULTI-STOCK HONEST EVALUATION ===")
print(f"Universe: {sorted(test_df.tic.unique().tolist())}")
print(f"Train {TRAIN_START} to {TRAIN_END} | Val {VAL_START} to {VAL_END} | Test {TEST_START} to {TEST_END}")
print(f"Cost: {COST*100:.2f}% per trade")
print("Metric             | RL PPO (10 stocks) | Equal-weight Buy&Hold")
